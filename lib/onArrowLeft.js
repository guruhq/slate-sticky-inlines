...
function handleArrowLeftOutsideInline(event, change, editor, opts) {
  ...
  const isAtStartOfCurrentTextNode = change.value.selection.focus.offset === 0;
  ...
  if (
    isInlineBanned(change.value.schema, upcomingNode, opts) ||
    !hasStickyBoundaries ||
    isExtending ||
    upcomingNode.isVoid ||
    textNodeIndex === -1
...
function handleArrowLeftInsideInline(event, change, editor, opts) {
  ...
  const isAtSecondToFirstCharacter = change.value.selection.focus.offset === 1;

  // Thanks to this very plugin, it's common to be in this change.value where you are at the edge of an inline.
  const isAtFirstCharacter = change.value.selection.focus.offset === 0;
...
export default function onArrowLeft(event, change, editor, opts) {
  ...
  if (change.value.focusInline)
    return handleArrowLeftInsideInline(event, change, editor, opts);
