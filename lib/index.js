...
function StickyInlines(opts) {
  ...
  function onKeyDown(event, change, editor) {
    ...
    if (
      ...
      isInlineBanned(change.value.schema, change.value.focusInline, opts)
    )
      return null;
  ...
  function onChange(change) {
    ...
    const toRemove = change.value.document
      .getInlines()
      .reduce((failures, inline) => {
        const hasFocus =
          change.value.selection.isFocused &&
          change.value.selection.anchor.isInNode(inline);
        const onlyHasZeroWidthSpace = inline.text === ZERO_WIDTH_SPACE;

        if (isInlineBanned(change.value.schema, inline, opts)) return failures;
    ...
    toRemove.forEach(
      (failure) => (change = change.removeNodeByKey(failure.key))
    );
    return true;
  }
  ...
  function onSelect(event, change) {
    ...
    const focusInline = change.value.document.getClosestInline(
      selection.anchor.key
    );
    if (!focusInline) return null;

    const selectionIsAtEndOfInline =
      focusInline.key === change.value.focusInline.key &&
      selection.focus.offset === focusInline.text.length;
