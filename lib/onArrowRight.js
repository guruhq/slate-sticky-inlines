...
function handleArrowRightOutsideInline(event, change, editor, opts) {
  ...
  const isAtEndOfCurrentTextNode =
    change.value.selection.focus.offset === change.value.focusText.text.length;
  ...
  if (
    ...
    !hasStickyBoundaries ||
    isExtending ||
    upcomingNode.isVoid // need to change?
  )
    return null;
...
function handleArrowRightInsideInline(event, change, editor, opts) {
  ...
  const isAtSecondToLastCharacter =
    change.value.selection.focus.offset ===
  ...
  const isAtLastCharacter =
    change.value.selection.focus.offset ===
    change.value.focusInline.text.length;
...
export default function onArrowRight(event, change, editor, opts) {
  ...
  if (change.value.focusInline)
    return handleArrowRightInsideInline(event, change, editor, opts);
