...
function handleArrowLeftOutsideInline(event, change, editor, opts) {
  ...
  var isAtStartOfCurrentTextNode = change.value.selection.focusOffset === 0;
  ...
  if ((0, _utils.isInlineBanned)(upcomingNode, opts) || !hasStickyBoundaries || isExtending || upcomingNode.isVoid || textNodeIndex === -1) return null;
...
function handleArrowLeftInsideInline(event, change, editor, opts) {
  ...
  var isAtSecondToFirstCharacter = change.value.selection.focusOffset === 1;

  // Thanks to this very plugin, it's common to be in this change.value where you are at the edge of an inline.
  var isAtFirstCharacter = change.value.selection.focusOffset === 0;
